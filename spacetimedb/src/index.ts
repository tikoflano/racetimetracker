  ...
  for (const v of ctx.db.venue.iter()) {
    ...
    for (const t of ctx.db.track.iter()) {
      ...
      for (const tv of ctx.db.track_variation.iter()) {
        if (tv.track_id === t.id) ctx.db.track_variation.id.delete(tv.id);
      }
...
export const add_event_member = spacetimedb.reducer(
  ...
  (ctx, args) => {
    ...
    if (args.role !== 'organizer' && args.role !== 'timekeeper')
      throw new SenderError('Invalid role');
    for (const m of ctx.db.event_member.iter()) {
...
export const seed_demo_data = spacetimedb.reducer((ctx) => {
  ...
  const pendingData = [
    ...
    {
      email: 'pending-timekeeper@example.com',
      name: 'Pending Timekeeper',
      role: 'timekeeper',
    },
  ];
