...
export function AppSidebar({ collapsed, onClose }: AppSidebarProps) {
  ...
  return (
    <>
      {isMobile && !collapsed && <div className={classes.backdrop} onClick={onClose} />}
      <nav className={`${classes.sidebar} ${collapsed ? classes.sidebarCollapsed : ''}`}>
        {/* Logo */}
        <div className={classes.logo}>
          <div className={classes.logoIcon}>
            <IconClock size={18} stroke={2} />
          </div>
          <span className={`${classes.logoText} ${collapsed ? classes.logoTextHidden : ''}`}>
            RaceTimeTracker
          </span>
        </div>
